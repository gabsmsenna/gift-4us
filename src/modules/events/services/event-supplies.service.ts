...
import type { CreateEventSupplyDto } from '../dtos/create-event-supply.dto';
import type { UpdateEventSupplyDto } from '../dtos/update-event-supply.dto';
import type { CreateSupplyContributionDto } from '../dtos/create-supply-contribution.dto';
import type { UpdateSupplyContributionDto } from '../dtos/update-supply-contribution.dto';
import type { EventSupplyWithProgressDto } from '../dtos/event-supply-with-progress.dto';
...
export class EventSuppliesService {
  ...
  async createSupply(
    ...
    try {
      ...
      if (
        event.eventType !== EventType.REGISTRY &&
        event.eventType !== EventType.POTLUCK
      ) {
      ...
      if (error instanceof QueryFailedError) {
        this.logger.error(
          `Database error creating supply: ${error.message}`,
          error.stack,
        );
        throw new BadRequestException(
          'Erro ao criar suprimento: dados inválidos',
        );
      }

      // Log unexpected errors
      this.logger.error(
        `Unexpected error creating supply: ${error.message}`,
        error.stack,
      );
  ...
  async getEventSupplies(
    eventId: string,
  ): Promise<EventSupplyWithProgressDto[]> {
    const cacheKey = this.getCacheKey(eventId);

    try {
      // Check cache first using getCacheKey helper
      const cachedData =
        await this.cacheManager.get<EventSupplyWithProgressDto[]>(cacheKey);
      ...
      this.logger.warn(
        `Cache read failed for ${cacheKey}: ${cacheError.message}`,
      );
    }
    ...
    try {
      ...
      const suppliesWithProgress: EventSupplyWithProgressDto[] = supplies.map(
        (supply) => {
          // Calculate quantityCommitted by summing contribution quantities
          const quantityCommitted = this.calculateTotalCommitted(
      ...
      if (error instanceof QueryFailedError) {
        this.logger.error(
          `Database error fetching supplies: ${error.message}`,
          error.stack,
        );
        throw new BadRequestException(
          'Erro ao buscar suprimentos: problema no banco de dados',
        );
      }

      // Log unexpected errors
      this.logger.error(
        `Unexpected error fetching supplies: ${error.message}`,
        error.stack,
      );
  ...
  async updateSupply(
    ...
    try {
      ...
      if (dto.quantityNeeded !== undefined)
        supply.quantityNeeded = dto.quantityNeeded;
      ...
      if (error instanceof QueryFailedError) {
        this.logger.error(
          `Database error updating supply: ${error.message}`,
          error.stack,
        );
        throw new BadRequestException(
          'Erro ao atualizar suprimento: dados inválidos',
        );
      }

      // Log unexpected errors
      this.logger.error(
        `Unexpected error updating supply: ${error.message}`,
        error.stack,
      );
  ...
  async deleteSupply(supplyId: string, userId: string): Promise<void> {
    try {
      ...
      if (error instanceof QueryFailedError) {
        this.logger.error(
          `Database error deleting supply: ${error.message}`,
          error.stack,
        );
        throw new BadRequestException(
          'Erro ao deletar suprimento: problema no banco de dados',
        );
      }

      // Log unexpected errors
      this.logger.error(
        `Unexpected error deleting supply: ${error.message}`,
        error.stack,
      );
  ...
  async createContribution(
    ...
    try {
      ...
      const savedContribution =
        await this.supplyContributionRepository.save(contribution);
      ...
      if (error instanceof QueryFailedError) {
        this.logger.error(
          `Database error creating contribution: ${error.message}`,
          error.stack,
        );
        throw new BadRequestException(
          'Erro ao criar contribuição: dados inválidos',
        );
      }
      ...
      if (error instanceof Error) {
        this.logger.error(
          `Unexpected error creating contribution: ${error.message}`,
          error.stack,
        );
      }
  ...
  async getSupplyContributions(
    ...
    try {
      ...
      if (error instanceof QueryFailedError) {
        this.logger.error(
          `Database error fetching contributions: ${error.message}`,
          error.stack,
        );
        throw new BadRequestException(
          'Erro ao buscar contribuições: problema no banco de dados',
        );
      }
      ...
      this.logger.error(
        `Unexpected error fetching contributions: ${message}`,
        stack,
      );
  ...
  async updateContribution(
    ...
    try {
      ...
      if (error instanceof QueryFailedError) {
        this.logger.error(
          `Database error updating contribution: ${error.message}`,
          error.stack,
        );
        throw new BadRequestException(
          'Erro ao atualizar contribuição: dados inválidos',
        );
      }
      ...
      this.logger.error(
        `Unexpected error updating contribution: ${message}`,
        stack,
      );
  ...
  async deleteContribution(
    ...
    try {
      ...
      if (error instanceof QueryFailedError) {
        this.logger.error(
          `Database error deleting contribution: ${error.message}`,
          error.stack,
        );
        throw new BadRequestException(
          'Erro ao deletar contribuição: problema no banco de dados',
        );
      }
      ...
      if (error instanceof Error) {
        this.logger.error(
          `Unexpected error deleting contribution: ${error.message}`,
          error.stack,
        );
  ...
  private async verifyEventOwnerOrAdmin(
    eventId: string,
    userId: string,
  ): Promise<void> {
    try {
      ...
      throw new ForbiddenException(
        'Apenas o dono do evento ou administradores do grupo podem criar suprimentos',
      );
    } catch (error) {
      // Re-throw known exceptions
      if (
      ...
      if (error instanceof QueryFailedError) {
        this.logger.error(
          `Database error verifying event owner/admin: ${error.message}`,
          error.stack,
        );
      ...
      if (error instanceof Error) {
        this.logger.error(
          `Unexpected error verifying event owner/admin: ${error.message}`,
          error.stack,
        );
  ...
  private async verifyEventParticipant(
    eventId: string,
    userId: string,
  ): Promise<void> {
    try {
      ...
      if (!isParticipant) {
        throw new ForbiddenException(
          'Apenas participantes do evento podem criar contribuições',
        );
      }
    } catch (error) {
      if (
      ...
      if (error instanceof QueryFailedError) {
        this.logger.error(
          `Database error verifying event participant: ${error.message}`,
          error.stack,
        );
        throw new BadRequestException('Erro ao verificar permissões');
      }
      ...
      if (error instanceof Error) {
        this.logger.error(
          `Unexpected error verifying event participant: ${error.message}`,
          error.stack,
        );
        throw new ForbiddenException('Erro ao verificar permissões');
      }
  ...
  private async verifyContributionOwner(
    contributionId: string,
    userId: string,
  ): Promise<void> {
    try {
      ...
      if (contribution.userId !== userId) {
        throw new ForbiddenException(
          'Você não tem permissão para modificar esta contribuição',
        );
      }
    } catch (error) {
      // Re-throw known exceptions
      ...
      if (error instanceof QueryFailedError) {
        this.logger.error(
          `Database error verifying contribution owner: ${error.message}`,
          error.stack,
        );
        throw new BadRequestException('Erro ao verificar permissões');
      }
      ...
      if (error instanceof Error) {
        this.logger.error(
          `Unexpected error verifying contribution owner: ${error.message}`,
          error.stack,
        );
        throw new ForbiddenException('Erro ao verificar permissões');
      }
  ...
  private async validateContributionQuantity(
    ...
    try {
      ...
      if (error instanceof QueryFailedError) {
        this.logger.error(
          `Database error validating contribution quantity: ${error.message}`,
          error.stack,
        );
        throw new BadRequestException(
          'Erro ao validar quantidade da contribuição',
        );
      }
      ...
      this.logger.error(
        `Unexpected error validating contribution quantity: ${message}`,
        stack,
      );
      throw new BadRequestException(
        'Erro ao validar quantidade da contribuição',
  ...
  private calculateTotalCommitted(
    ...
    return contributions
      ...
      .reduce(
        (total, contribution) => total + contribution.quantityCommitted,
        0,
      );
  }
  ...
  private async invalidateSuppliesCache(eventId: string): Promise<void> {
    ...
    try {
      ...
      if (cacheError instanceof Error) {
        this.logger.warn(
          `Cache deletion failed for ${cacheKey}: ${cacheError.message}`,
        );
      }
    }
  ...
  private async publishCacheInvalidation(
    ...
    try {
      ...
      const errorMessage =
        rabbitError instanceof Error
          ? rabbitError.message
          : String(rabbitError);
      this.logger.warn(
        `RabbitMQ publish failed (attempt ${attempt}/${maxRetries}): ${errorMessage}`,
      );
      ...
      if (attempt < maxRetries) {
        ...
        this.logger.error(
          `RabbitMQ publish failed after ${maxRetries} attempts. Cache invalidation message lost.`,
        );
      }
    }
  }
